...
export default function ImageSingle({ imageUrl, isLoading }: ImageSingleProps) {
  return (
    // 바깥: 스크롤 영역 안에서 남는 공간만큼만 차지하는 슬롯. 넘치면 스크롤이 아니라 안쪽 프레임이 줄어듦
    <div className="relative flex-1 min-h-0 flex items-center justify-center">
      {/* 안쪽: 이 슬롯의 가로/세로 중 더 빠듯한 쪽에 맞춰 비율(1264:848)을 유지한 채 최대로 커짐 */}
      <div className="relative max-w-full max-h-full min-w-125 aspect-1264/848 overflow-hidden rounded-xl border border-border bg-linear-to-br from-[#3B2478] via-[#141634] to-[#1B2A63] flex items-center justify-center">
        {isLoading && !imageUrl ? (
          ...
          // 프레임 비율이 이미지 비율과 정확히 같으므로, 그냥 꽉 채우면 여백도 잘림도 없음
