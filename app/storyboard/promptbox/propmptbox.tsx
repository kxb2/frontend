  ...
  return (
    <div className="shrink-0 rounded-xl border border-border bg-surface p-4">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={handleCopy} className="flex items-center gap-1 rounded-full border border-border px-3 py-1.5 text-xs text-text-secondary">
      ...
      <div className={`mt-3 whitespace-pre-wrap rounded-lg border border-border bg-background p-3 text-xs text-text-secondary ${isExpanded ? '' : 'line-clamp-3'}`}>{promptText ?? '프롬프트'}</div>

      <button type="button" onClick={() => setIsExpanded((prev) => !prev)} className="mt-2 text-xs text-text-secondary">
        {isExpanded ? '접기 ↑' : '더보기 ↓'}
      </button>
    </div>
  );
